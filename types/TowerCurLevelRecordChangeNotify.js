...
class TowerCurLevelRecordChangeNotify$Type extends runtime_5.MessageType {
    constructor() {
        super("TowerCurLevelRecordChangeNotify", [
            { no: 1, name: "cur_level_record", kind: "message", T: () => TowerCurLevelRecord_1.TowerCurLevelRecord }
        ]);
    }
    create(value) {
        const message = {};
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        if (message.curLevelRecord)
            TowerCurLevelRecord_1.TowerCurLevelRecord.internalBinaryWrite(message.curLevelRecord, writer.tag(1, runtime_1.WireType.LengthDelimited).fork(), options).join();
