...
class GetPlayerBlacklistRsp$Type extends runtime_5.MessageType {
    constructor() {
        super("GetPlayerBlacklistRsp", [
            { no: 5, name: "blacklist", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => FriendBrief_1.FriendBrief },
            { no: 11, name: "retcode", kind: "scalar", T: 5 /*ScalarType.INT32*/ }
        ]);
    }
    create(value) {
        const message = { blacklist: [], retcode: 0 };
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        for (let i = 0; i < message.blacklist.length; i++)
            FriendBrief_1.FriendBrief.internalBinaryWrite(message.blacklist[i], writer.tag(5, runtime_1.WireType.LengthDelimited).fork(), options).join();
        /* int32 retcode = 11; */
