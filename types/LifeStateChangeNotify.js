...
class LifeStateChangeNotify$Type extends runtime_5.MessageType {
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        if (message.moveReliableSeq !== 0)
            writer.tag(7, runtime_1.WireType.Varint).uint32(message.moveReliableSeq);
        /* uint32 entity_id = 8; */
        if (message.entityId !== 0)
            writer.tag(8, runtime_1.WireType.Varint).uint32(message.entityId);
        /* uint32 life_state = 10; */
        if (message.lifeState !== 0)
            writer.tag(10, runtime_1.WireType.Varint).uint32(message.lifeState);
        /* repeated ServerBuff server_buff_list = 15; */
        for (let i = 0; i < message.serverBuffList.length; i++)
            ServerBuff_1.ServerBuff.internalBinaryWrite(message.serverBuffList[i], writer.tag(15, runtime_1.WireType.LengthDelimited).fork(), options).join();
        ...
        if (message.attackTag !== "")
            writer.tag(13, runtime_1.WireType.LengthDelimited).string(message.attackTag);
