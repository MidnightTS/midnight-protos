...
class HuntingGiveUpReq$Type extends runtime_5.MessageType {
    constructor() {
        super("HuntingGiveUpReq", [
            { no: 2, name: "hunting_pair", kind: "message", T: () => HuntingPair_1.HuntingPair }
        ]);
    }
    create(value) {
        const message = {};
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        if (message.huntingPair)
            HuntingPair_1.HuntingPair.internalBinaryWrite(message.huntingPair, writer.tag(2, runtime_1.WireType.LengthDelimited).fork(), options).join();
