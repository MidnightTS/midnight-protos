...
export interface EntityConfigHashNotify {
    /**
     * @generated from protobuf field: repeated EntityConfigHashEntry avatar_entry_list = 1;
     */
    avatarEntryList: EntityConfigHashEntry[];
    /**
     * @generated from protobuf field: repeated EntityConfigHashEntry combat_entry_list = 2;
     */
    combatEntryList: EntityConfigHashEntry[];
    /**
     * @generated from protobuf field: repeated EntityConfigHashEntry ability_entry_list = 9;
     */
...
class EntityConfigHashNotify$Type extends MessageType<EntityConfigHashNotify> {
    constructor() {
        super("EntityConfigHashNotify", [
            { no: 1, name: "avatar_entry_list", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => EntityConfigHashEntry },
            { no: 2, name: "combat_entry_list", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => EntityConfigHashEntry },
            { no: 9, name: "ability_entry_list", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => EntityConfigHashEntry }
        ]);
    }
    ...
    internalBinaryWrite(message: EntityConfigHashNotify, writer: IBinaryWriter, options: BinaryWriteOptions): IBinaryWriter {
        ...
        for (let i = 0; i < message.avatarEntryList.length; i++)
            EntityConfigHashEntry.internalBinaryWrite(message.avatarEntryList[i], writer.tag(1, WireType.LengthDelimited).fork(), options).join();
        /* repeated EntityConfigHashEntry combat_entry_list = 2; */
        for (let i = 0; i < message.combatEntryList.length; i++)
            EntityConfigHashEntry.internalBinaryWrite(message.combatEntryList[i], writer.tag(2, WireType.LengthDelimited).fork(), options).join();
        /* repeated EntityConfigHashEntry ability_entry_list = 9; */
        for (let i = 0; i < message.abilityEntryList.length; i++)
            EntityConfigHashEntry.internalBinaryWrite(message.abilityEntryList[i], writer.tag(9, WireType.LengthDelimited).fork(), options).join();
