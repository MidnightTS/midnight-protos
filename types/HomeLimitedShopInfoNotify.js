...
class HomeLimitedShopInfoNotify$Type extends runtime_5.MessageType {
    constructor() {
        super("HomeLimitedShopInfoNotify", [
            { no: 7, name: "shop_info", kind: "message", T: () => HomeLimitedShopInfo_1.HomeLimitedShopInfo }
        ]);
    }
    create(value) {
        const message = {};
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        if (message.shopInfo)
            HomeLimitedShopInfo_1.HomeLimitedShopInfo.internalBinaryWrite(message.shopInfo, writer.tag(7, runtime_1.WireType.LengthDelimited).fork(), options).join();
