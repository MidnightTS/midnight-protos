...
class PlayerRandomCookReq$Type extends runtime_5.MessageType {
    constructor() {
        super("PlayerRandomCookReq", [
            { no: 5, name: "material_list", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => ItemParam_1.ItemParam }
        ]);
    }
    create(value) {
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                case /* repeated ItemParam material_list */ 5:
                    message.materialList.push(ItemParam_1.ItemParam.internalBinaryRead(reader, reader.uint32(), options));
                    break;
                default:
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        for (let i = 0; i < message.materialList.length; i++)
            ItemParam_1.ItemParam.internalBinaryWrite(message.materialList[i], writer.tag(5, runtime_1.WireType.LengthDelimited).fork(), options).join();
