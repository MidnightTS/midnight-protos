...
class SetUpLunchBoxWidgetReq$Type extends runtime_5.MessageType {
    constructor() {
        super("SetUpLunchBoxWidgetReq", [
            { no: 7, name: "lunch_box_data", kind: "message", T: () => LunchBoxData_1.LunchBoxData }
        ]);
    }
    create(value) {
        const message = {};
    ...
    internalBinaryRead(reader, length, options, target) {
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                case /* LunchBoxData lunch_box_data */ 7:
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        if (message.lunchBoxData)
            LunchBoxData_1.LunchBoxData.internalBinaryWrite(message.lunchBoxData, writer.tag(7, runtime_1.WireType.LengthDelimited).fork(), options).join();
