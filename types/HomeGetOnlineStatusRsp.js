...
class HomeGetOnlineStatusRsp$Type extends runtime_5.MessageType {
    constructor() {
        super("HomeGetOnlineStatusRsp", [
            ...
            { no: 2, name: "player_info_list", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => OnlinePlayerInfo_1.OnlinePlayerInfo }
        ]);
    }
    create(value) {
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        for (let i = 0; i < message.playerInfoList.length; i++)
            OnlinePlayerInfo_1.OnlinePlayerInfo.internalBinaryWrite(message.playerInfoList[i], writer.tag(2, runtime_1.WireType.LengthDelimited).fork(), options).join();
