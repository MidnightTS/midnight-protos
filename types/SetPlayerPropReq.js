...
class SetPlayerPropReq$Type extends runtime_5.MessageType {
    constructor() {
        super("SetPlayerPropReq", [
            { no: 3, name: "prop_list", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => PropValue_1.PropValue }
        ]);
    }
    create(value) {
    ...
    internalBinaryRead(reader, length, options, target) {
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                case /* repeated PropValue prop_list */ 3:
                    message.propList.push(PropValue_1.PropValue.internalBinaryRead(reader, reader.uint32(), options));
                    break;
                default:
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        for (let i = 0; i < message.propList.length; i++)
            PropValue_1.PropValue.internalBinaryWrite(message.propList[i], writer.tag(3, runtime_1.WireType.LengthDelimited).fork(), options).join();
