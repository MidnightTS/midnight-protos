...
class ClientAbilityInitFinishNotify$Type extends runtime_5.MessageType {
    constructor() {
        super("ClientAbilityInitFinishNotify", [
            ...
            { no: 9, name: "invokes", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => AbilityInvokeEntry_1.AbilityInvokeEntry }
        ]);
    }
    create(value) {
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        for (let i = 0; i < message.invokes.length; i++)
            AbilityInvokeEntry_1.AbilityInvokeEntry.internalBinaryWrite(message.invokes[i], writer.tag(9, runtime_1.WireType.LengthDelimited).fork(), options).join();
