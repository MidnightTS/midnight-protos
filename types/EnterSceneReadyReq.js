...
class EnterSceneReadyReq$Type extends runtime_5.MessageType {
    constructor() {
        super("EnterSceneReadyReq", [
            { no: 12, name: "enter_scene_token", kind: "scalar", T: 13 /*ScalarType.UINT32*/ }
        ]);
    }
    create(value) {
    ...
    internalBinaryRead(reader, length, options, target) {
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                case /* uint32 enter_scene_token */ 12:
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        if (message.enterSceneToken !== 0)
            writer.tag(12, runtime_1.WireType.Varint).uint32(message.enterSceneToken);
