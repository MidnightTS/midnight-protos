/**
 * Generated by the protoc-gen-ts.  DO NOT EDIT!
 * compiler version: 3.20.3
 * source: GetMapMarkTipsRsp.proto
 * git: https://github.com/thesayyn/protoc-gen-ts */
import * as dependency_1 from "./MapMarkTipsInfo";
import * as pb_1 from "google-protobuf";
export class GetMapMarkTipsRsp extends pb_1.Message {
    #one_of_decls: number[][] = [];
    constructor(data?: any[] | {
        retcode?: number;
        mark_tips_list?: dependency_1.MapMarkTipsInfo[];
    }) {
        super();
        pb_1.Message.initialize(this, Array.isArray(data) ? data : [], 0, -1, [11], this.#one_of_decls);
        if (!Array.isArray(data) && typeof data == "object") {
            if ("retcode" in data && data.retcode != undefined) {
                this.retcode = data.retcode;
            }
            if ("mark_tips_list" in data && data.mark_tips_list != undefined) {
                this.mark_tips_list = data.mark_tips_list;
            }
        }
    }
    get retcode() {
        return pb_1.Message.getFieldWithDefault(this, 7, 0) as number;
    }
    set retcode(value: number) {
        pb_1.Message.setField(this, 7, value);
    }
    get mark_tips_list() {
        return pb_1.Message.getRepeatedWrapperField(this, dependency_1.MapMarkTipsInfo, 11) as dependency_1.MapMarkTipsInfo[];
    }
    set mark_tips_list(value: dependency_1.MapMarkTipsInfo[]) {
        pb_1.Message.setRepeatedWrapperField(this, 11, value);
    }
    static fromObject(data: {
        retcode?: number;
        mark_tips_list?: ReturnType<typeof dependency_1.MapMarkTipsInfo.prototype.toObject>[];
    }): GetMapMarkTipsRsp {
        const message = new GetMapMarkTipsRsp({});
        if (data.retcode != null) {
            message.retcode = data.retcode;
        }
        if (data.mark_tips_list != null) {
            message.mark_tips_list = data.mark_tips_list.map(item => dependency_1.MapMarkTipsInfo.fromObject(item));
        }
        return message;
    }
    toObject() {
        const data: {
            retcode?: number;
            mark_tips_list?: ReturnType<typeof dependency_1.MapMarkTipsInfo.prototype.toObject>[];
        } = {};
        if (this.retcode != null) {
            data.retcode = this.retcode;
        }
        if (this.mark_tips_list != null) {
            data.mark_tips_list = this.mark_tips_list.map((item: dependency_1.MapMarkTipsInfo) => item.toObject());
        }
        return data;
    }
    serialize(): Uint8Array;
    serialize(w: pb_1.BinaryWriter): void;
    serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
        const writer = w || new pb_1.BinaryWriter();
        if (this.retcode != 0)
            writer.writeInt32(7, this.retcode);
        if (this.mark_tips_list.length)
            writer.writeRepeatedMessage(11, this.mark_tips_list, (item: dependency_1.MapMarkTipsInfo) => item.serialize(writer));
        if (!w)
            return writer.getResultBuffer();
    }
    static deserialize(bytes: Uint8Array | pb_1.BinaryReader): GetMapMarkTipsRsp {
        const reader = bytes instanceof pb_1.BinaryReader ? bytes : new pb_1.BinaryReader(bytes), message = new GetMapMarkTipsRsp();
        while (reader.nextField()) {
            if (reader.isEndGroup())
                break;
            switch (reader.getFieldNumber()) {
                case 7:
                    message.retcode = reader.readInt32();
                    break;
                case 11:
                    reader.readMessage(message.mark_tips_list, () => pb_1.Message.addToRepeatedWrapperField(message, 11, dependency_1.MapMarkTipsInfo.deserialize(reader), dependency_1.MapMarkTipsInfo));
                    break;
                default: reader.skipField();
            }
        }
        return message;
    }
    serializeBinary(): Uint8Array {
        return this.serialize();
    }
    static deserializeBinary(bytes: Uint8Array): GetMapMarkTipsRsp {
        return GetMapMarkTipsRsp.deserialize(bytes);
    }
}
