...
const ShowAvatarInfo_1 = require("./ShowAvatarInfo");
// @generated message type with reflection information, may provide speed optimized methods
class GetFriendShowAvatarInfoRsp$Type extends runtime_5.MessageType {
    constructor() {
        super("GetFriendShowAvatarInfoRsp", [
            { no: 5, name: "show_avatar_info_list", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => ShowAvatarInfo_1.ShowAvatarInfo },
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                case /* repeated ShowAvatarInfo show_avatar_info_list */ 5:
                    message.showAvatarInfoList.push(ShowAvatarInfo_1.ShowAvatarInfo.internalBinaryRead(reader, reader.uint32(), options));
                    break;
                case /* uint32 uid */ 9:
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        for (let i = 0; i < message.showAvatarInfoList.length; i++)
            ShowAvatarInfo_1.ShowAvatarInfo.internalBinaryWrite(message.showAvatarInfoList[i], writer.tag(5, runtime_1.WireType.LengthDelimited).fork(), options).join();
        /* uint32 uid = 9; */
        if (message.uid !== 0)
