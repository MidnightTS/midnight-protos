...
class AvatarFightPropUpdateNotify$Type extends runtime_5.MessageType {
    constructor() {
        super("AvatarFightPropUpdateNotify", [
            ...
            { no: 10, name: "fight_prop_map", kind: "map", K: 13 /*ScalarType.UINT32*/, V: { kind: "scalar", T: 2 /*ScalarType.FLOAT*/ } }
        ]);
    }
    create(value) {
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                case /* uint64 avatar_guid */ 13:
                    ...
                    break;
                case /* map<uint32, float> fight_prop_map */ 10:
                    this.binaryReadMap10(message.fightPropMap, reader, options);
                    break;
                default:
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        for (let k of Object.keys(message.fightPropMap))
            writer.tag(10, runtime_1.WireType.LengthDelimited).fork().tag(1, runtime_1.WireType.Varint).uint32(parseInt(k)).tag(2, runtime_1.WireType.Bit32).float(message.fightPropMap[k]).join();
