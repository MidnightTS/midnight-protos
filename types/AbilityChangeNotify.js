...
class AbilityChangeNotify$Type extends runtime_5.MessageType {
    constructor() {
        super("AbilityChangeNotify", [
            { no: 6, name: "ability_control_block", kind: "message", T: () => AbilityControlBlock_1.AbilityControlBlock },
    ...
    internalBinaryRead(reader, length, options, target) {
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                case /* AbilityControlBlock ability_control_block */ 6:
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        if (message.abilityControlBlock)
            AbilityControlBlock_1.AbilityControlBlock.internalBinaryWrite(message.abilityControlBlock, writer.tag(6, runtime_1.WireType.LengthDelimited).fork(), options).join();
        /* uint32 entity_id = 10; */
