...
exports.RemotePlayerWidgetNotify = void 0;
...
class RemotePlayerWidgetNotify$Type extends runtime_5.MessageType {
    constructor() {
        super("RemotePlayerWidgetNotify", [
            { no: 10, name: "player_widget_info_list", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => PlayerWidgetInfo_1.PlayerWidgetInfo }
        ]);
    }
    create(value) {
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        for (let i = 0; i < message.playerWidgetInfoList.length; i++)
            PlayerWidgetInfo_1.PlayerWidgetInfo.internalBinaryWrite(message.playerWidgetInfoList[i], writer.tag(10, runtime_1.WireType.LengthDelimited).fork(), options).join();
