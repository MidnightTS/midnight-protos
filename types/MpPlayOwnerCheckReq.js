"use strict";
var __classPrivateFieldGet = (this && this.__classPrivateFieldGet) || function (receiver, state, kind, f) {
    if (kind === "a" && !f) throw new TypeError("Private accessor was defined without a getter");
    if (typeof state === "function" ? receiver !== state || !f : !state.has(receiver)) throw new TypeError("Cannot read private member from an object whose class did not declare it");
    return kind === "m" ? f : kind === "a" ? f.call(receiver) : f ? f.value : state.get(receiver);
};
var _MpPlayOwnerCheckReq_one_of_decls;
Object.defineProperty(exports, "__esModule", { value: true });
exports.MpPlayOwnerCheckReq = void 0;
/**
 * Generated by the protoc-gen-ts.  DO NOT EDIT!
 * compiler version: 3.20.3
 * source: MpPlayOwnerCheckReq.proto
 * git: https://github.com/thesayyn/protoc-gen-ts */
const pb_1 = require("google-protobuf");
class MpPlayOwnerCheckReq extends pb_1.Message {
    constructor(data) {
        super();
        _MpPlayOwnerCheckReq_one_of_decls.set(this, []);
        pb_1.Message.initialize(this, Array.isArray(data) ? data : [], 0, -1, [], __classPrivateFieldGet(this, _MpPlayOwnerCheckReq_one_of_decls, "f"));
        if (!Array.isArray(data) && typeof data == "object") {
            if ("mp_play_id" in data && data.mp_play_id != undefined) {
                this.mp_play_id = data.mp_play_id;
            }
            if ("is_skip_match" in data && data.is_skip_match != undefined) {
                this.is_skip_match = data.is_skip_match;
            }
        }
    }
    get mp_play_id() {
        return pb_1.Message.getFieldWithDefault(this, 9, 0);
    }
    set mp_play_id(value) {
        pb_1.Message.setField(this, 9, value);
    }
    get is_skip_match() {
        return pb_1.Message.getFieldWithDefault(this, 3, false);
    }
    set is_skip_match(value) {
        pb_1.Message.setField(this, 3, value);
    }
    static fromObject(data) {
        const message = new MpPlayOwnerCheckReq({});
        if (data.mp_play_id != null) {
            message.mp_play_id = data.mp_play_id;
        }
        if (data.is_skip_match != null) {
            message.is_skip_match = data.is_skip_match;
        }
        return message;
    }
    toObject() {
        const data = {};
        if (this.mp_play_id != null) {
            data.mp_play_id = this.mp_play_id;
        }
        if (this.is_skip_match != null) {
            data.is_skip_match = this.is_skip_match;
        }
        return data;
    }
    serialize(w) {
        const writer = w || new pb_1.BinaryWriter();
        if (this.mp_play_id != 0)
            writer.writeUint32(9, this.mp_play_id);
        if (this.is_skip_match != false)
            writer.writeBool(3, this.is_skip_match);
        if (!w)
            return writer.getResultBuffer();
    }
    static deserialize(bytes) {
        const reader = bytes instanceof pb_1.BinaryReader ? bytes : new pb_1.BinaryReader(bytes), message = new MpPlayOwnerCheckReq();
        while (reader.nextField()) {
            if (reader.isEndGroup())
                break;
            switch (reader.getFieldNumber()) {
                case 9:
                    message.mp_play_id = reader.readUint32();
                    break;
                case 3:
                    message.is_skip_match = reader.readBool();
                    break;
                default: reader.skipField();
            }
        }
        return message;
    }
    serializeBinary() {
        return this.serialize();
    }
    static deserializeBinary(bytes) {
        return MpPlayOwnerCheckReq.deserialize(bytes);
    }
}
exports.MpPlayOwnerCheckReq = MpPlayOwnerCheckReq;
_MpPlayOwnerCheckReq_one_of_decls = new WeakMap();
