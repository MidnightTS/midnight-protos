...
class WaterSpiritActivityDetailInfo$Type extends runtime_5.MessageType {
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        for (let k of Object.keys(message.searchTimeMap))
            writer.tag(13, runtime_1.WireType.LengthDelimited).fork().tag(1, runtime_1.WireType.Varint).uint32(parseInt(k)).tag(2, runtime_1.WireType.Varint).uint32(message.searchTimeMap[k]).join();
        /* uint32 region_search_id = 4; */
        if (message.regionSearchId !== 0)
            writer.tag(4, runtime_1.WireType.Varint).uint32(message.regionSearchId);
        /* uint32 mp_play_id = 9; */
