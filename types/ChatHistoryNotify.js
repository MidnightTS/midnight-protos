...
class ChatHistoryNotify$Type extends runtime_5.MessageType {
    constructor() {
        super("ChatHistoryNotify", [
            { no: 1, name: "channel_id", kind: "scalar", T: 13 /*ScalarType.UINT32*/ },
            { no: 9, name: "chat_info", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => ChatInfo_1.ChatInfo }
        ]);
    }
    create(value) {
        const message = { channelId: 0, chatInfo: [] };
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                case /* uint32 channel_id */ 1:
                    ...
                    break;
                case /* repeated ChatInfo chat_info */ 9:
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        for (let i = 0; i < message.chatInfo.length; i++)
            ChatInfo_1.ChatInfo.internalBinaryWrite(message.chatInfo[i], writer.tag(9, runtime_1.WireType.LengthDelimited).fork(), options).join();
...
/**
 * @generated MessageType for protobuf message ChatHistoryNotify
 */
