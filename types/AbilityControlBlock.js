...
const AbilityEmbryo_1 = require("./AbilityEmbryo");
...
class AbilityControlBlock$Type extends runtime_5.MessageType {
    constructor() {
        super("com.midnights.game.AbilityControlBlock", [
            { no: 1, name: "ability_embryo_list", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => AbilityEmbryo_1.AbilityEmbryo }
        ]);
    }
    create(value) {
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                case /* repeated com.midnights.game.AbilityEmbryo ability_embryo_list */ 1:
                    message.abilityEmbryoList.push(AbilityEmbryo_1.AbilityEmbryo.internalBinaryRead(reader, reader.uint32(), options));
                    break;
                default:
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        for (let i = 0; i < message.abilityEmbryoList.length; i++)
            AbilityEmbryo_1.AbilityEmbryo.internalBinaryWrite(message.abilityEmbryoList[i], writer.tag(1, runtime_1.WireType.LengthDelimited).fork(), options).join();
