"use strict";
var __classPrivateFieldGet = (this && this.__classPrivateFieldGet) || function (receiver, state, kind, f) {
    if (kind === "a" && !f) throw new TypeError("Private accessor was defined without a getter");
    if (typeof state === "function" ? receiver !== state || !f : !state.has(receiver)) throw new TypeError("Cannot read private member from an object whose class did not declare it");
    return kind === "m" ? f : kind === "a" ? f.call(receiver) : f ? f.value : state.get(receiver);
};
var _InBattleMechanicusSettleInfo_one_of_decls;
Object.defineProperty(exports, "__esModule", { value: true });
exports.InBattleMechanicusSettleInfo = void 0;
/**
 * Generated by the protoc-gen-ts.  DO NOT EDIT!
 * compiler version: 3.20.3
 * source: InBattleMechanicusSettleInfo.proto
 * git: https://github.com/thesayyn/protoc-gen-ts */
const dependency_1 = require("./MultistageSettleWatcherInfo");
const pb_1 = require("google-protobuf");
class InBattleMechanicusSettleInfo extends pb_1.Message {
    constructor(data) {
        super();
        _InBattleMechanicusSettleInfo_one_of_decls.set(this, []);
        pb_1.Message.initialize(this, Array.isArray(data) ? data : [], 0, -1, [7], __classPrivateFieldGet(this, _InBattleMechanicusSettleInfo_one_of_decls, "f"));
        if (!Array.isArray(data) && typeof data == "object") {
            if ("scene_time_ms" in data && data.scene_time_ms != undefined) {
                this.scene_time_ms = data.scene_time_ms;
            }
            if ("total_token" in data && data.total_token != undefined) {
                this.total_token = data.total_token;
            }
            if ("real_token" in data && data.real_token != undefined) {
                this.real_token = data.real_token;
            }
            if ("watcher_list" in data && data.watcher_list != undefined) {
                this.watcher_list = data.watcher_list;
            }
            if ("is_success" in data && data.is_success != undefined) {
                this.is_success = data.is_success;
            }
            if ("play_index" in data && data.play_index != undefined) {
                this.play_index = data.play_index;
            }
            if ("difficulty_percentage" in data && data.difficulty_percentage != undefined) {
                this.difficulty_percentage = data.difficulty_percentage;
            }
            if ("group_id" in data && data.group_id != undefined) {
                this.group_id = data.group_id;
            }
        }
    }
    get scene_time_ms() {
        return pb_1.Message.getFieldWithDefault(this, 15, 0);
    }
    set scene_time_ms(value) {
        pb_1.Message.setField(this, 15, value);
    }
    get total_token() {
        return pb_1.Message.getFieldWithDefault(this, 4, 0);
    }
    set total_token(value) {
        pb_1.Message.setField(this, 4, value);
    }
    get real_token() {
        return pb_1.Message.getFieldWithDefault(this, 8, 0);
    }
    set real_token(value) {
        pb_1.Message.setField(this, 8, value);
    }
    get watcher_list() {
        return pb_1.Message.getRepeatedWrapperField(this, dependency_1.MultistageSettleWatcherInfo, 7);
    }
    set watcher_list(value) {
        pb_1.Message.setRepeatedWrapperField(this, 7, value);
    }
    get is_success() {
        return pb_1.Message.getFieldWithDefault(this, 6, false);
    }
    set is_success(value) {
        pb_1.Message.setField(this, 6, value);
    }
    get play_index() {
        return pb_1.Message.getFieldWithDefault(this, 3, 0);
    }
    set play_index(value) {
        pb_1.Message.setField(this, 3, value);
    }
    get difficulty_percentage() {
        return pb_1.Message.getFieldWithDefault(this, 10, 0);
    }
    set difficulty_percentage(value) {
        pb_1.Message.setField(this, 10, value);
    }
    get group_id() {
        return pb_1.Message.getFieldWithDefault(this, 13, 0);
    }
    set group_id(value) {
        pb_1.Message.setField(this, 13, value);
    }
    static fromObject(data) {
        const message = new InBattleMechanicusSettleInfo({});
        if (data.scene_time_ms != null) {
            message.scene_time_ms = data.scene_time_ms;
        }
        if (data.total_token != null) {
            message.total_token = data.total_token;
        }
        if (data.real_token != null) {
            message.real_token = data.real_token;
        }
        if (data.watcher_list != null) {
            message.watcher_list = data.watcher_list.map(item => dependency_1.MultistageSettleWatcherInfo.fromObject(item));
        }
        if (data.is_success != null) {
            message.is_success = data.is_success;
        }
        if (data.play_index != null) {
            message.play_index = data.play_index;
        }
        if (data.difficulty_percentage != null) {
            message.difficulty_percentage = data.difficulty_percentage;
        }
        if (data.group_id != null) {
            message.group_id = data.group_id;
        }
        return message;
    }
    toObject() {
        const data = {};
        if (this.scene_time_ms != null) {
            data.scene_time_ms = this.scene_time_ms;
        }
        if (this.total_token != null) {
            data.total_token = this.total_token;
        }
        if (this.real_token != null) {
            data.real_token = this.real_token;
        }
        if (this.watcher_list != null) {
            data.watcher_list = this.watcher_list.map((item) => item.toObject());
        }
        if (this.is_success != null) {
            data.is_success = this.is_success;
        }
        if (this.play_index != null) {
            data.play_index = this.play_index;
        }
        if (this.difficulty_percentage != null) {
            data.difficulty_percentage = this.difficulty_percentage;
        }
        if (this.group_id != null) {
            data.group_id = this.group_id;
        }
        return data;
    }
    serialize(w) {
        const writer = w || new pb_1.BinaryWriter();
        if (this.scene_time_ms != 0)
            writer.writeUint64(15, this.scene_time_ms);
        if (this.total_token != 0)
            writer.writeUint32(4, this.total_token);
        if (this.real_token != 0)
            writer.writeUint32(8, this.real_token);
        if (this.watcher_list.length)
            writer.writeRepeatedMessage(7, this.watcher_list, (item) => item.serialize(writer));
        if (this.is_success != false)
            writer.writeBool(6, this.is_success);
        if (this.play_index != 0)
            writer.writeUint32(3, this.play_index);
        if (this.difficulty_percentage != 0)
            writer.writeUint32(10, this.difficulty_percentage);
        if (this.group_id != 0)
            writer.writeUint32(13, this.group_id);
        if (!w)
            return writer.getResultBuffer();
    }
    static deserialize(bytes) {
        const reader = bytes instanceof pb_1.BinaryReader ? bytes : new pb_1.BinaryReader(bytes), message = new InBattleMechanicusSettleInfo();
        while (reader.nextField()) {
            if (reader.isEndGroup())
                break;
            switch (reader.getFieldNumber()) {
                case 15:
                    message.scene_time_ms = reader.readUint64();
                    break;
                case 4:
                    message.total_token = reader.readUint32();
                    break;
                case 8:
                    message.real_token = reader.readUint32();
                    break;
                case 7:
                    reader.readMessage(message.watcher_list, () => pb_1.Message.addToRepeatedWrapperField(message, 7, dependency_1.MultistageSettleWatcherInfo.deserialize(reader), dependency_1.MultistageSettleWatcherInfo));
                    break;
                case 6:
                    message.is_success = reader.readBool();
                    break;
                case 3:
                    message.play_index = reader.readUint32();
                    break;
                case 10:
                    message.difficulty_percentage = reader.readUint32();
                    break;
                case 13:
                    message.group_id = reader.readUint32();
                    break;
                default: reader.skipField();
            }
        }
        return message;
    }
    serializeBinary() {
        return this.serialize();
    }
    static deserializeBinary(bytes) {
        return InBattleMechanicusSettleInfo.deserialize(bytes);
    }
}
exports.InBattleMechanicusSettleInfo = InBattleMechanicusSettleInfo;
_InBattleMechanicusSettleInfo_one_of_decls = new WeakMap();
