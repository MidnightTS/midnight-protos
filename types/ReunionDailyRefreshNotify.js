...
class ReunionDailyRefreshNotify$Type extends runtime_5.MessageType {
    constructor() {
        super("ReunionDailyRefreshNotify", [
            { no: 6, name: "reunion_brief_info", kind: "message", T: () => ReunionBriefInfo_1.ReunionBriefInfo }
        ]);
    }
    create(value) {
        const message = {};
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        if (message.reunionBriefInfo)
            ReunionBriefInfo_1.ReunionBriefInfo.internalBinaryWrite(message.reunionBriefInfo, writer.tag(6, runtime_1.WireType.LengthDelimited).fork(), options).join();
