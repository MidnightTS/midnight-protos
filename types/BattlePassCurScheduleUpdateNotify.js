...
class BattlePassCurScheduleUpdateNotify$Type extends runtime_5.MessageType {
    constructor() {
        super("BattlePassCurScheduleUpdateNotify", [
            { no: 2, name: "cur_schedule", kind: "message", T: () => BattlePassSchedule_1.BattlePassSchedule },
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        if (message.curSchedule)
            BattlePassSchedule_1.BattlePassSchedule.internalBinaryWrite(message.curSchedule, writer.tag(2, runtime_1.WireType.LengthDelimited).fork(), options).join();
        /* bool have_cur_schedule = 12; */
        if (message.haveCurSchedule !== false)
            writer.tag(12, runtime_1.WireType.Varint).bool(message.haveCurSchedule);
