...
class GroupSuiteNotify$Type extends runtime_5.MessageType {
    constructor() {
        super("GroupSuiteNotify", [
            { no: 4, name: "group_map", kind: "map", K: 13 /*ScalarType.UINT32*/, V: { kind: "scalar", T: 13 /*ScalarType.UINT32*/ } }
        ]);
    }
    create(value) {
    ...
    internalBinaryRead(reader, length, options, target) {
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                case /* map<uint32, uint32> group_map */ 4:
                    this.binaryReadMap4(message.groupMap, reader, options);
                    break;
                default:
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        for (let k of Object.keys(message.groupMap))
            writer.tag(4, runtime_1.WireType.LengthDelimited).fork().tag(1, runtime_1.WireType.Varint).uint32(parseInt(k)).tag(2, runtime_1.WireType.Varint).uint32(message.groupMap[k]).join();
