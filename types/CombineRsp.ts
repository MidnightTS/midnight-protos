// @generated by protobuf-ts 2.8.1 with parameter output_typescript
// @generated from protobuf file "CombineRsp.proto" (syntax proto3)
// tslint:disable
//
// Sorapointa - A server software re-implementation for a certain anime game, and avoid sorapointa.
// Copyright (C) 2022  Sorapointa Team
//
// This program is free software: you can redistribute it and/or modify
// it under the terms of the GNU Affero General Public License as
// published by the Free Software Foundation, either version 3 of the
// License, or (at your option) any later version.
//
// This program is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU Affero General Public License for more details.
//
// You should have received a copy of the GNU Affero General Public License
// along with this program.  If not, see <https://www.gnu.org/licenses/>.
//
import type { BinaryWriteOptions } from "@protobuf-ts/runtime";
import type { IBinaryWriter } from "@protobuf-ts/runtime";
import { WireType } from "@protobuf-ts/runtime";
import type { BinaryReadOptions } from "@protobuf-ts/runtime";
import type { IBinaryReader } from "@protobuf-ts/runtime";
import { UnknownFieldHandler } from "@protobuf-ts/runtime";
import type { PartialMessage } from "@protobuf-ts/runtime";
import { reflectionMergePartial } from "@protobuf-ts/runtime";
import { MESSAGE_TYPE } from "@protobuf-ts/runtime";
import { MessageType } from "@protobuf-ts/runtime";
import { ItemParam } from "./ItemParam";
/**
 * enum CmdId {
 *   option allow_alias = true;
 *   NONE = 0;
 *   CMD_ID = 634;
 *   ENET_CHANNEL_ID = 0;
 *   ENET_IS_RELIABLE = 1;
 * }
 *
 * @generated from protobuf message CombineRsp
 */
export interface CombineRsp {
    /**
     * @generated from protobuf field: repeated ItemParam Unk3300_IAMGGFHOMEC = 11 [json_name = "Unk3300IAMGGFHOMEC"];
     */
    unk3300IAMGGFHOMEC: ItemParam[];
    /**
     * @generated from protobuf field: repeated ItemParam Unk3300_BAKPEKIFJOA = 13 [json_name = "Unk3300BAKPEKIFJOA"];
     */
    unk3300BAKPEKIFJOA: ItemParam[];
    /**
     * @generated from protobuf field: repeated ItemParam Unk3300_CDAFJDFHLKI = 6 [json_name = "Unk3300CDAFJDFHLKI"];
     */
    unk3300CDAFJDFHLKI: ItemParam[];
    /**
     * @generated from protobuf field: uint64 avatar_guid = 7;
     */
    avatarGuid: bigint;
    /**
     * @generated from protobuf field: uint32 combine_count = 2;
     */
    combineCount: number;
    /**
     * @generated from protobuf field: uint32 combine_id = 10;
     */
    combineId: number;
    /**
     * @generated from protobuf field: repeated ItemParam cost_item_list = 15;
     */
    costItemList: ItemParam[];
    /**
     * @generated from protobuf field: int32 retcode = 1;
     */
    retcode: number;
    /**
     * @generated from protobuf field: repeated ItemParam Unk3300_FLHNPFGNNBM = 4 [json_name = "Unk3300FLHNPFGNNBM"];
     */
    unk3300FLHNPFGNNBM: ItemParam[];
}
// @generated message type with reflection information, may provide speed optimized methods
class CombineRsp$Type extends MessageType<CombineRsp> {
    constructor() {
        super("CombineRsp", [
            { no: 11, name: "Unk3300_IAMGGFHOMEC", kind: "message", jsonName: "Unk3300IAMGGFHOMEC", repeat: 1 /*RepeatType.PACKED*/, T: () => ItemParam },
            { no: 13, name: "Unk3300_BAKPEKIFJOA", kind: "message", jsonName: "Unk3300BAKPEKIFJOA", repeat: 1 /*RepeatType.PACKED*/, T: () => ItemParam },
            { no: 6, name: "Unk3300_CDAFJDFHLKI", kind: "message", jsonName: "Unk3300CDAFJDFHLKI", repeat: 1 /*RepeatType.PACKED*/, T: () => ItemParam },
            { no: 7, name: "avatar_guid", kind: "scalar", T: 4 /*ScalarType.UINT64*/, L: 0 /*LongType.BIGINT*/ },
            { no: 2, name: "combine_count", kind: "scalar", T: 13 /*ScalarType.UINT32*/ },
            { no: 10, name: "combine_id", kind: "scalar", T: 13 /*ScalarType.UINT32*/ },
            { no: 15, name: "cost_item_list", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => ItemParam },
            { no: 1, name: "retcode", kind: "scalar", T: 5 /*ScalarType.INT32*/ },
            { no: 4, name: "Unk3300_FLHNPFGNNBM", kind: "message", jsonName: "Unk3300FLHNPFGNNBM", repeat: 1 /*RepeatType.PACKED*/, T: () => ItemParam }
        ]);
    }
    create(value?: PartialMessage<CombineRsp>): CombineRsp {
        const message = { unk3300IAMGGFHOMEC: [], unk3300BAKPEKIFJOA: [], unk3300CDAFJDFHLKI: [], avatarGuid: 0n, combineCount: 0, combineId: 0, costItemList: [], retcode: 0, unk3300FLHNPFGNNBM: [] };
        globalThis.Object.defineProperty(message, MESSAGE_TYPE, { enumerable: false, value: this });
        if (value !== undefined)
            reflectionMergePartial<CombineRsp>(this, message, value);
        return message;
    }
    internalBinaryRead(reader: IBinaryReader, length: number, options: BinaryReadOptions, target?: CombineRsp): CombineRsp {
        let message = target ?? this.create(), end = reader.pos + length;
        while (reader.pos < end) {
            let [fieldNo, wireType] = reader.tag();
            switch (fieldNo) {
                case /* repeated ItemParam Unk3300_IAMGGFHOMEC = 11 [json_name = "Unk3300IAMGGFHOMEC"];*/ 11:
                    message.unk3300IAMGGFHOMEC.push(ItemParam.internalBinaryRead(reader, reader.uint32(), options));
                    break;
                case /* repeated ItemParam Unk3300_BAKPEKIFJOA = 13 [json_name = "Unk3300BAKPEKIFJOA"];*/ 13:
                    message.unk3300BAKPEKIFJOA.push(ItemParam.internalBinaryRead(reader, reader.uint32(), options));
                    break;
                case /* repeated ItemParam Unk3300_CDAFJDFHLKI = 6 [json_name = "Unk3300CDAFJDFHLKI"];*/ 6:
                    message.unk3300CDAFJDFHLKI.push(ItemParam.internalBinaryRead(reader, reader.uint32(), options));
                    break;
                case /* uint64 avatar_guid */ 7:
                    message.avatarGuid = reader.uint64().toBigInt();
                    break;
                case /* uint32 combine_count */ 2:
                    message.combineCount = reader.uint32();
                    break;
                case /* uint32 combine_id */ 10:
                    message.combineId = reader.uint32();
                    break;
                case /* repeated ItemParam cost_item_list */ 15:
                    message.costItemList.push(ItemParam.internalBinaryRead(reader, reader.uint32(), options));
                    break;
                case /* int32 retcode */ 1:
                    message.retcode = reader.int32();
                    break;
                case /* repeated ItemParam Unk3300_FLHNPFGNNBM = 4 [json_name = "Unk3300FLHNPFGNNBM"];*/ 4:
                    message.unk3300FLHNPFGNNBM.push(ItemParam.internalBinaryRead(reader, reader.uint32(), options));
                    break;
                default:
                    let u = options.readUnknownField;
                    if (u === "throw")
                        throw new globalThis.Error(`Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`);
                    let d = reader.skip(wireType);
                    if (u !== false)
                        (u === true ? UnknownFieldHandler.onRead : u)(this.typeName, message, fieldNo, wireType, d);
            }
        }
        return message;
    }
    internalBinaryWrite(message: CombineRsp, writer: IBinaryWriter, options: BinaryWriteOptions): IBinaryWriter {
        /* repeated ItemParam Unk3300_IAMGGFHOMEC = 11 [json_name = "Unk3300IAMGGFHOMEC"]; */
        for (let i = 0; i < message.unk3300IAMGGFHOMEC.length; i++)
            ItemParam.internalBinaryWrite(message.unk3300IAMGGFHOMEC[i], writer.tag(11, WireType.LengthDelimited).fork(), options).join();
        /* repeated ItemParam Unk3300_BAKPEKIFJOA = 13 [json_name = "Unk3300BAKPEKIFJOA"]; */
        for (let i = 0; i < message.unk3300BAKPEKIFJOA.length; i++)
            ItemParam.internalBinaryWrite(message.unk3300BAKPEKIFJOA[i], writer.tag(13, WireType.LengthDelimited).fork(), options).join();
        /* repeated ItemParam Unk3300_CDAFJDFHLKI = 6 [json_name = "Unk3300CDAFJDFHLKI"]; */
        for (let i = 0; i < message.unk3300CDAFJDFHLKI.length; i++)
            ItemParam.internalBinaryWrite(message.unk3300CDAFJDFHLKI[i], writer.tag(6, WireType.LengthDelimited).fork(), options).join();
        /* uint64 avatar_guid = 7; */
        if (message.avatarGuid !== 0n)
            writer.tag(7, WireType.Varint).uint64(message.avatarGuid);
        /* uint32 combine_count = 2; */
        if (message.combineCount !== 0)
            writer.tag(2, WireType.Varint).uint32(message.combineCount);
        /* uint32 combine_id = 10; */
        if (message.combineId !== 0)
            writer.tag(10, WireType.Varint).uint32(message.combineId);
        /* repeated ItemParam cost_item_list = 15; */
        for (let i = 0; i < message.costItemList.length; i++)
            ItemParam.internalBinaryWrite(message.costItemList[i], writer.tag(15, WireType.LengthDelimited).fork(), options).join();
        /* int32 retcode = 1; */
        if (message.retcode !== 0)
            writer.tag(1, WireType.Varint).int32(message.retcode);
        /* repeated ItemParam Unk3300_FLHNPFGNNBM = 4 [json_name = "Unk3300FLHNPFGNNBM"]; */
        for (let i = 0; i < message.unk3300FLHNPFGNNBM.length; i++)
            ItemParam.internalBinaryWrite(message.unk3300FLHNPFGNNBM[i], writer.tag(4, WireType.LengthDelimited).fork(), options).join();
        let u = options.writeUnknownFields;
        if (u !== false)
            (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
        return writer;
    }
}
/**
 * @generated MessageType for protobuf message CombineRsp
 */
export const CombineRsp = new CombineRsp$Type();
