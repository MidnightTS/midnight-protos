...
class WidgetGadgetDataNotify$Type extends runtime_5.MessageType {
    constructor() {
        super("WidgetGadgetDataNotify", [
            { no: 3, name: "widget_gadget_data", kind: "message", T: () => WidgetGadgetData_1.WidgetGadgetData }
        ]);
    }
    create(value) {
        const message = {};
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                case /* WidgetGadgetData widget_gadget_data */ 3:
                    message.widgetGadgetData = WidgetGadgetData_1.WidgetGadgetData.internalBinaryRead(reader, reader.uint32(), options, message.widgetGadgetData);
                    break;
                default:
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        if (message.widgetGadgetData)
            WidgetGadgetData_1.WidgetGadgetData.internalBinaryWrite(message.widgetGadgetData, writer.tag(3, runtime_1.WireType.LengthDelimited).fork(), options).join();
