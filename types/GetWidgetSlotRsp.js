...
class GetWidgetSlotRsp$Type extends runtime_5.MessageType {
    constructor() {
        super("GetWidgetSlotRsp", [
            { no: 3, name: "slot_list", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => WidgetSlotData_1.WidgetSlotData },
            { no: 10, name: "retcode", kind: "scalar", T: 5 /*ScalarType.INT32*/ }
        ]);
    }
    create(value) {
        const message = { slotList: [], retcode: 0 };
    ...
    internalBinaryRead(reader, length, options, target) {
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                case /* repeated WidgetSlotData slot_list */ 3:
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        for (let i = 0; i < message.slotList.length; i++)
            WidgetSlotData_1.WidgetSlotData.internalBinaryWrite(message.slotList[i], writer.tag(3, runtime_1.WireType.LengthDelimited).fork(), options).join();
        /* int32 retcode = 10; */
