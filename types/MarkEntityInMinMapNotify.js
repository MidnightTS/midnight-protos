...
class MarkEntityInMinMapNotify$Type extends runtime_5.MessageType {
    constructor() {
        super("MarkEntityInMinMapNotify", [
            { no: 4, name: "position", kind: "message", T: () => Vector_1.Vector },
            { no: 3, name: "entity_id", kind: "scalar", T: 13 /*ScalarType.UINT32*/ },
            { no: 14, name: "monster_id", kind: "scalar", T: 13 /*ScalarType.UINT32*/ }
        ]);
    }
    create(value) {
    ...
    internalBinaryWrite(message, writer, options) {
        /* Vector position = 4; */
        if (message.position)
            Vector_1.Vector.internalBinaryWrite(message.position, writer.tag(4, runtime_1.WireType.LengthDelimited).fork(), options).join();
        /* uint32 entity_id = 3; */
