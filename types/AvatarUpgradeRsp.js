...
class AvatarUpgradeRsp$Type extends runtime_5.MessageType {
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        for (let k of Object.keys(message.curFightPropMap))
            writer.tag(13, runtime_1.WireType.LengthDelimited).fork().tag(1, runtime_1.WireType.Varint).uint32(parseInt(k)).tag(2, runtime_1.WireType.Bit32).float(message.curFightPropMap[k]).join();
        /* map<uint32, float> old_fight_prop_map = 6; */
        for (let k of Object.keys(message.oldFightPropMap))
            writer.tag(6, runtime_1.WireType.LengthDelimited).fork().tag(1, runtime_1.WireType.Varint).uint32(parseInt(k)).tag(2, runtime_1.WireType.Bit32).float(message.oldFightPropMap[k]).join();
        /* uint32 old_level = 12; */
