...
class GetPlayerFriendListRsp$Type extends runtime_5.MessageType {
    constructor() {
        super("GetPlayerFriendListRsp", [
            { no: 13, name: "ask_friend_list", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => FriendBrief_1.FriendBrief },
            { no: 4, name: "retcode", kind: "scalar", T: 5 /*ScalarType.INT32*/ },
            { no: 15, name: "friend_list", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => FriendBrief_1.FriendBrief }
        ]);
    }
    create(value) {
        const message = { askFriendList: [], retcode: 0, friendList: [] };
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                ...
                case /* int32 retcode */ 4:
                    ...
                    break;
                case /* repeated FriendBrief friend_list */ 15:
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        for (let i = 0; i < message.askFriendList.length; i++)
            FriendBrief_1.FriendBrief.internalBinaryWrite(message.askFriendList[i], writer.tag(13, runtime_1.WireType.LengthDelimited).fork(), options).join();
        /* int32 retcode = 4; */
        ...
        for (let i = 0; i < message.friendList.length; i++)
            FriendBrief_1.FriendBrief.internalBinaryWrite(message.friendList[i], writer.tag(15, runtime_1.WireType.LengthDelimited).fork(), options).join();
