/**
 * Generated by the protoc-gen-ts.  DO NOT EDIT!
 * compiler version: 3.20.3
 * source: SceneEntityMoveNotify.proto
 * git: https://github.com/thesayyn/protoc-gen-ts */
import * as dependency_1 from "./MotionInfo";
import * as pb_1 from "google-protobuf";
export class SceneEntityMoveNotify extends pb_1.Message {
    #one_of_decls: number[][] = [];
    constructor(data?: any[] | {
        motion_info?: dependency_1.MotionInfo;
        entity_id?: number;
        scene_time?: number;
        reliable_seq?: number;
    }) {
        super();
        pb_1.Message.initialize(this, Array.isArray(data) ? data : [], 0, -1, [], this.#one_of_decls);
        if (!Array.isArray(data) && typeof data == "object") {
            if ("motion_info" in data && data.motion_info != undefined) {
                this.motion_info = data.motion_info;
            }
            if ("entity_id" in data && data.entity_id != undefined) {
                this.entity_id = data.entity_id;
            }
            if ("scene_time" in data && data.scene_time != undefined) {
                this.scene_time = data.scene_time;
            }
            if ("reliable_seq" in data && data.reliable_seq != undefined) {
                this.reliable_seq = data.reliable_seq;
            }
        }
    }
    get motion_info() {
        return pb_1.Message.getWrapperField(this, dependency_1.MotionInfo, 6) as dependency_1.MotionInfo;
    }
    set motion_info(value: dependency_1.MotionInfo) {
        pb_1.Message.setWrapperField(this, 6, value);
    }
    get has_motion_info() {
        return pb_1.Message.getField(this, 6) != null;
    }
    get entity_id() {
        return pb_1.Message.getFieldWithDefault(this, 8, 0) as number;
    }
    set entity_id(value: number) {
        pb_1.Message.setField(this, 8, value);
    }
    get scene_time() {
        return pb_1.Message.getFieldWithDefault(this, 15, 0) as number;
    }
    set scene_time(value: number) {
        pb_1.Message.setField(this, 15, value);
    }
    get reliable_seq() {
        return pb_1.Message.getFieldWithDefault(this, 2, 0) as number;
    }
    set reliable_seq(value: number) {
        pb_1.Message.setField(this, 2, value);
    }
    static fromObject(data: {
        motion_info?: ReturnType<typeof dependency_1.MotionInfo.prototype.toObject>;
        entity_id?: number;
        scene_time?: number;
        reliable_seq?: number;
    }): SceneEntityMoveNotify {
        const message = new SceneEntityMoveNotify({});
        if (data.motion_info != null) {
            message.motion_info = dependency_1.MotionInfo.fromObject(data.motion_info);
        }
        if (data.entity_id != null) {
            message.entity_id = data.entity_id;
        }
        if (data.scene_time != null) {
            message.scene_time = data.scene_time;
        }
        if (data.reliable_seq != null) {
            message.reliable_seq = data.reliable_seq;
        }
        return message;
    }
    toObject() {
        const data: {
            motion_info?: ReturnType<typeof dependency_1.MotionInfo.prototype.toObject>;
            entity_id?: number;
            scene_time?: number;
            reliable_seq?: number;
        } = {};
        if (this.motion_info != null) {
            data.motion_info = this.motion_info.toObject();
        }
        if (this.entity_id != null) {
            data.entity_id = this.entity_id;
        }
        if (this.scene_time != null) {
            data.scene_time = this.scene_time;
        }
        if (this.reliable_seq != null) {
            data.reliable_seq = this.reliable_seq;
        }
        return data;
    }
    serialize(): Uint8Array;
    serialize(w: pb_1.BinaryWriter): void;
    serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
        const writer = w || new pb_1.BinaryWriter();
        if (this.has_motion_info)
            writer.writeMessage(6, this.motion_info, () => this.motion_info.serialize(writer));
        if (this.entity_id != 0)
            writer.writeUint32(8, this.entity_id);
        if (this.scene_time != 0)
            writer.writeUint32(15, this.scene_time);
        if (this.reliable_seq != 0)
            writer.writeUint32(2, this.reliable_seq);
        if (!w)
            return writer.getResultBuffer();
    }
    static deserialize(bytes: Uint8Array | pb_1.BinaryReader): SceneEntityMoveNotify {
        const reader = bytes instanceof pb_1.BinaryReader ? bytes : new pb_1.BinaryReader(bytes), message = new SceneEntityMoveNotify();
        while (reader.nextField()) {
            if (reader.isEndGroup())
                break;
            switch (reader.getFieldNumber()) {
                case 6:
                    reader.readMessage(message.motion_info, () => message.motion_info = dependency_1.MotionInfo.deserialize(reader));
                    break;
                case 8:
                    message.entity_id = reader.readUint32();
                    break;
                case 15:
                    message.scene_time = reader.readUint32();
                    break;
                case 2:
                    message.reliable_seq = reader.readUint32();
                    break;
                default: reader.skipField();
            }
        }
        return message;
    }
    serializeBinary(): Uint8Array {
        return this.serialize();
    }
    static deserializeBinary(bytes: Uint8Array): SceneEntityMoveNotify {
        return SceneEntityMoveNotify.deserialize(bytes);
    }
}
