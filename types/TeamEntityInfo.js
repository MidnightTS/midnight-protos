...
class TeamEntityInfo$Type extends runtime_5.MessageType {
    constructor() {
        super("TeamEntityInfo", [
            ...
            { no: 5, name: "team_ability_info", kind: "message", T: () => AbilitySyncStateInfo_1.AbilitySyncStateInfo }
        ]);
    }
    create(value) {
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        if (message.teamEntityId !== 0)
            writer.tag(3, runtime_1.WireType.Varint).uint32(message.teamEntityId);
        /* uint32 authority_peer_id = 12; */
        if (message.authorityPeerId !== 0)
            writer.tag(12, runtime_1.WireType.Varint).uint32(message.authorityPeerId);
        /* AbilitySyncStateInfo team_ability_info = 5; */
        if (message.teamAbilityInfo)
            AbilitySyncStateInfo_1.AbilitySyncStateInfo.internalBinaryWrite(message.teamAbilityInfo, writer.tag(5, runtime_1.WireType.LengthDelimited).fork(), options).join();
